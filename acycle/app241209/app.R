...
library(plotly)
library(gridlayout)
#interactively when wd is not in app dir
# source('acycle/app241209/c-cleanlib.R')      
# source('acycle/app241209/rctree.R')
# load('acycle/app241209/.RData') #see app7  
# load('acycle/app241209/pxosrdo2dd.RData')  
#in app
source('c-cleanlib.R')      
source('rctree.R')
load('.RData') #contains f241921ad x101 z110 pxosrdo2dd
#load('pxosrdo2dd.RData')  incorporated in .Rdata
...
grid_page( ###grid page ----
           ...
           grid_card(
             ...
             card_body(
               ...
               #   card_body(
               conditionalPanel(
                 condition="output.comrc6==output.selrc6",#"all.equal('A','B')",
               ...
               actionButton("docusabC", "Re-estimate custom"),
             )
           ),
           grid_card(
             area = "sidebar",
             ...
             card_body(
               ...
               #   inputId = "rc6tC",
               #   label = "District",
               #   choices=irregpcode(sort(unique(substr(dir('smallrip/'),1,6)))),
               #   selected = irregpcode(rc6cuC[1])
               # ),
               treeInput( #districts
                 inputId = "rc6tC",
               ...
               #   )
               # ),
               span(textOutput('selrc6'), style="color:black"), #white
               span(textOutput('comrc6'), style="color:black") #white
             )
           ),#grid card end
           grid_card_text(
             area = "header",
...
function(input, output) {
  ...
  # iv$add_rule("rc6tC", sv_required())
  # iv$add_rule("rc6tC", function(value) {
  # if(
  #   (irregpcode(regpcode(input$rc6tC))!=input$rc6tC)|(nchar(regpcode(input$rc6tC))!=6)
  #   ) {"Postcode district required"}
  # })
  # iv$enable()
  selectedrc6R<- reactive({
    x <- paste0(paste0(sort(unique(input$rctreeC)),collapse=','))
    selectedrc6G <<- copy(x)
    x
  })
  ...
  output$selrc6 <- renderText({ selectedrc6R() })
  output$comrc6 <- renderText({ computedrc6R() })
  ...
  #output$cuseqcom <- renderText(span({'custom indices match selection'}, style="size:small"))#color:white
  ...
  #rsicuG$kfoldsse[,rc6]%>%irregpcode(.)%>%paste(.,collapse=',')
  ...
  observe(
    x={
      ...
      rc6c <- f241208fd[rc6tR()==target,f240920b(id)]
      if(length(rc6c)>0) {
        if(verbose) {
  ...
  )
  {
    ...
    x <- x2%>%
      ...
      #ggtitle(geoqX[,paste0('Custom districts: ',paste0(sort(irregpcode(rc6)),collapse=', '))],size=8)+
      xlab('')+
