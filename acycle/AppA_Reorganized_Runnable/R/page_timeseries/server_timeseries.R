server_timeseries <- function(input, output, session, common) {
  print("Server_timeseries")
output$debug_ts <- renderText({
  print("✅ timeseries server active")
  "Placeholder output"
})
  ...
  x111D <- eventReactive(
    list(common$rc6tR(), common$rc6cuR(), common$geoaR(), common$z110R()),
    {
      if (verbose) {
        print("enter x111D")
      }
      x <- f111D(
        ...
        geoaX = common$geoaR(),
        pxosrdo2ddX = pxosrdo2ddG,#common$pxosrdo2ddR(),
        z110X = common$z110R(),
