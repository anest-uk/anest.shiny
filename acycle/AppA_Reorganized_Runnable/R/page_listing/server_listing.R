server_listing <- function(input, output, session, common) {
  ...
  x211cuD <- #listing-2-custom-estdtcu,qeocu,dfn  ----
    eventReactive( 
    list(common$estdtcuR(), 
         common$geocuR(), 
         common$dfnxxR()
    ),
    {
      if (verbose) print("enter x211D")
      geox <- copy(common$geocuR())[, let(rc6, rc9)] # used for aggregation and label
      x <- f211D(
        estdtlX = common$estdtcuR(),
        geoqX = common$geocuR()[, .(nx, lab, rc6 = rc9)], # non-standard geo
        dfnxxX = common$dfnxxR(),
        typeX = "C"
      )
      x211cuG <<- copy(x)
