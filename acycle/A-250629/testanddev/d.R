f250727b <- function(
  ...
  x7 <- x3[x5]
  x8 <- x4[x5]
  x9 <- seq_len(length(x2) - 1) # all bin IDs
  ...
  x11 <- CJ(row_id = x6$row_id, interval = x9)

  x12 <- x11[x10, on = .(row_id, interval)]
  x12[is.na(value), value := 0]

x13 <- dcast(x12, row_id ~ interval, value.var = "value", fill = 0)

existing_bins <- intersect(as.character(x9), names(x13))
setnames(x13,
         old = existing_bins,
         new = as.character(x2[-1][as.integer(existing_bins)]))
