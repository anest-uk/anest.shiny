...
function(input, output) {
  ...
  ###############################################
  yyy111D <- eventReactive(list(rctaR(),rccuR(),geoaR(),pxosrdo2ddR(),z110R()),       #111 map ----
                         {
                           if(verbose) print('X - enter x111D')
                           x <-   f111D(
                             rctaX=rctaR(), #TAR darker shade : scalar rc6
                             rccuX=rccuR(), #CUS onch outline : vector rc6
                             geoaX=geoaR(), #TER rgb shade : {nx    rc6    rc3 qtile    lab}
  ...
  yyy112D <- eventReactive(list(input$tslider,estdtxR(),ylimR()),#112 x(t)----
                         {
