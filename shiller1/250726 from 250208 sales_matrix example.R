#do a sip with all records - this now done
print('SIP(NAC, NO EPC)')
nn='f220822ad'#SFN seasonal 
base='NOW'
  rc6exc=f221007a()
  rc3inc=f230306a()
  stepin=file.path('.',base,'ver001','01nac',fsep='\\')
  stepout=file.path('.',base,'ver001','10sal',fsep='\\')
  # stepin='ver001\\01nac'
  # stepout='ver001\\02sip'
  #getgd(nn)
  f220822ad=getlast('f220822ad')
  sfn=f220822ad 
  hashlen=16
  #nac=fread(file=file.path(stepin,'all.csv'))
  nac=fread(file=file.path(stepin,'all.csv'))
  x8 <- nac%>% 
    .[new_build=='N']%>%#since V143 reduces 28.33M to 25.33M
    .[idx=='0ok',.(
      idhash,
